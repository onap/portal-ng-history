...
public class ActionsService {
  ...
  /**
   * Retrieve actions for a given userId from the database and provide a list with
   * actions
   * 
   * @param userId        only actions for this <code>userId</code> should be
   *                      retrieved
   * @param page          which page should be retrieved from the list of actions.
   *                      From a user perspective the first page has the page
   *                      number 1.
   *                      In the response list the first page starts with 0.
   *                      Therefore, a subtraction is needed.
   * @param pageSize      length of the response list
   * @param showLastHours for which hours from the current time the actions should
   *                      be retrieved.
   * @param saveInterval  value will be part of the response action object. This
   *                      value is set in the application properties.
   *                      In the future this value can be provided from the
   *                      client.
   * @param xRequestId    from the request header. Will be used in an error log
   * @return If successful object with an item list of action objects and an item
   *         with the list count, otherwise Mono error
   */
  public Mono<ActionsListResponse> getActions(String userId, Integer page, Integer pageSize, Integer showLastHours,
      Integer saveInterval, String xRequestId) {
   ...
   * 
   * @param userId              the id of the user for which the action should be
   *                            stored
   * @param createActionRequest the action object which should be stored
   * @param saveInterval        value will be part of the response action object.
   *                            This value is set in the application properties.
   *                            In the future this value can be provided from the
   *                            client.
   * @param xRequestId          from the request header. Will be used in an error
   *                            log
   * @return If successful object with the stored action, otherwise Mono error
   */
  public Mono<ActionResponse> createActions(String userId, CreateActionRequest createActionRequest,
      Integer saveInterval, String xRequestId) {
   ...
   */
  public Mono<ActionsListResponse> listActions(Integer page, Integer pageSize, Integer showLastHours,
      Integer saveInterval, String xRequestId) {
  ...
  /**
   * Delete actions after hours. This service will be used in the cron job. The
   * job will be implemented with a separate user story.
   * 
   * @param deleteAfterHours hours after the actions should be deleted
   * @return If successful empty Mono object, otherwise Mono error
   */
